...
r = subprocess.run(f'''
                    ...
                    ansible -o -i {inventory_file} -m shell -a 'tmux send-keys -t {TERMINALID} " && do-release-upgrade -f DistUpgradeViewNonInteractive"' {nodes}
...
while nodes_list:
    ...
    for node in nodes_list:
        ...
        elif 'Continue [yN]' in lines[-1]:
            subprocess.run(f'''
                            ansible -o -i {inventory_file} -m shell -a "tmux send-keys -t {TERMINALID} 'y' enter" {node}
                            ''',
                            shell=True)
        else:
            subprocess.run(f'''
